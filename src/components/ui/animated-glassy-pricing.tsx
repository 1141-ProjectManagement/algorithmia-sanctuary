...
export const PricingCard = ({
  ...
  const cardClasses = `
    ...
    from-background/80 to-background/40 border border-primary/20
    ${isPopular ? 'scale-105 relative ring-2 ring-primary/40 from-background/90 to-background/60 border-primary/40 shadow-2xl' : ''}
  `;
  const buttonClasses = `
    mt-auto w-full py-2.5 rounded-xl font-semibold text-[14px] transition font-sans
    ${buttonVariant === 'primary' 
      ? 'bg-primary hover:bg-primary/90 text-primary-foreground' 
      : 'bg-muted/50 hover:bg-muted/70 text-foreground border border-border'
    }
  `;
  ...
  return (
    <div className={cardClasses}>
      ...
      <div className="mb-4">
        <h3 className="text-xl font-bold text-foreground font-['Cinzel']">{planName}</h3>
        <p className="text-sm text-muted-foreground mt-1">{description}</p>
      </div>
      <div className="mb-6 flex items-baseline gap-1">
        <span className="text-4xl font-bold text-primary">{price === '聯繫我們' ? '' : '$'}{price}</span>
        {price !== '聯繫我們' && <span className="text-muted-foreground text-sm">/終身</span>}
      </div>
      <hr className="border-border/50 mb-6" />
      <ul className="space-y-3 mb-8 flex-grow">
        {features.map((feature, index) => (
          <li key={index} className="flex items-start gap-2 text-sm text-muted-foreground">
            <CheckIcon className="w-4 h-4 text-primary flex-shrink-0 mt-0.5" /> {feature}
          </li>
        ))}
      </ul>
      <RippleButton onClick={onButtonClick} className={buttonClasses}>{buttonText}</RippleButton>
    </div>
  );
};
